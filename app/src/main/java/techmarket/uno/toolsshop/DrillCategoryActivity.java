...
public class DrillCategoryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ArrayAdapter<Drill> adapter = new ArrayAdapter<>(
                ...
                drills);
        //адаптер установить в ListView
        lvDrills.setAdapter(adapter);
        //необходимо в класса Drill переопределить метод toString чтобы выводился title


        lvDrills.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                //по position выберем дрель, на которую кликнули
